...
2026-09-10 数据质量纠正：9 月 9 日访客聚合受旧服务端铸 ID 与每分钟 Feed
探针污染，生产模型已从保留的 ODS 以 9 月 10 日为可信起点重建；原始事件和入库台账
保留。投影现在拒绝无客户端会话的 `visitor_first_seen`，报表日均访客只使用快照
生成日前的完整 UTC 日。
...
用户已确认一期结束；四项观察移交 [二期数据质量计划](docs/analytics-phase2-data-quality.md)，
作为二期起点，随后推进完整性、去重归因、指标对账、新鲜度与异常恢复。
离线演示不连接数据库：
